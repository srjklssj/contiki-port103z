...
void EXTI3_handler(void) __attribute__((interrupt));
...
static void ethernet_pollhandler(void)
{
	...
	status = Enc_Read(ESTAT);	// TODO something must be done here
	//printf("ethernet_pollhandler come in with ESTAT:%02x\r\n", status);
	//status = Enc_Read(EIR);
	//printf("ethernet_pollhandler come in with EIR:%02x\r\n", status);
	...
	//status = Enc_Read(EIR);
	//printf("ethernet_pollhandler come in with EIR:%02x\r\n", status);
	// if there still has packets in the buffer, we must poll the process again
	//if (status & EIR_PKTIF) {
	if (!ENC_INT_PIN())
		process_poll(&ethernet_process);
